...
public class FileBrowser : MonoBehaviour
{
    ...
    public void ShowBrowser(string title, FinishedCallback callback, string startingDir, Rect screenRect,
        string okString)
    {
    ...
    public void ShowSaveBrowser(string title, FinishedCallback callback, string saveBaseDir, string saveDataDir,
        Rect screenRect, Texture2D screenshot)
    {
    ...
    {
        string saveBaseDir = this.defaultDir == ""
            ? Directory.GetCurrentDirectory() + Path.DirectorySeparatorChar + "saves"
            : this.defaultDir.FixPathSeparators();
    ...
    //
    public void ShowLoadBrowser(string title, FinishedCallback callback, string saveBaseDir, string saveDataDir,
        Rect screenRect)
    {
        this.dirExplorer = false;
        this.acceptsDirectories = false;
    ...
    {
        return FileBrowser.GetDefaultRect_();
    }
    ...
    void Start()
    {
        ...
        this.__AddSpecialFolder(Environment.SpecialFolder.MyDocuments, "My Docs", this.placesMyDocsIcon,
            ref _placesList);
        this.__AddSpecialFolder(Environment.SpecialFolder.MyPictures, "My Pics", this.placesMyPicsIcon,
            ref _placesList);
    ...
    private void __AddSpecialFolder(Environment.SpecialFolder folder, string name, Texture2D icon,
        ref List<PlacesData> placesList)
    {
    ...
    string MakeNewThumb(string filename, string tempName, int size)
    {
        ...
        try
        {
            using (Image<Rgba32> image = Image.Load(filename))
            {
                ...
                image.Save(newFilename); // Automatic encoder selected based on extension.
            }
    ...
    IEnumerator loadIcons()
    {
        for (int i = 0; i < this.filesWithImages.Length; i++)
        {
            ...
            if (!fileLower.EndsWith(".jpg") && !fileLower.EndsWith(".jpeg") && !fileLower.EndsWith(".png") &&
                !fileLower.EndsWith(".bmp") && !fileLower.EndsWith(".tga") && !fileLower.EndsWith(".tiff") &&
                !fileLower.EndsWith(".tif"))
            {
                continue;
            }
            ...
            filename = MakeNewThumb(filename, "tempThumb", 128);
    ...
    void UpdateSoundPreviewRect()
    {
        ...
        this.soundPreviewer.viewRect =
            new Rect(x / Screen.width, y / Screen.height, w / Screen.width, h / Screen.height);
    }
                    ...
                    if (this.selectedDirectory > 0)
                    {
                        ...
                        try
                        {
                            ...
                            myProcess.StartInfo.FileName =
                                Application.streamingAssetsPath.Replace("/", "\\") + "\\f2rb.exe";
    ...
    void OnGUI()
    {
        ...
        {
            if (Event.current.type == EventType.MouseDown && new Rect(this.screenRect.width + this.screenRect.x - 25,
                    this.screenRect.height + this.screenRect.y - 25, 25, 25).Contains(mousePos))
            {
                handleClicked = true;
            ...
            if (handleClicked)
            {
                ...
                {
                    this.screenRect.width = Mathf.Clamp(originalWindowRect.width + (mousePos.x - clickedPosition.x),
                        minWindowWidth, 1600);
                    this.screenRect.height = Mathf.Clamp(originalWindowRect.height + (mousePos.y - clickedPosition.y),
    ...
    IEnumerator FileSelected()
    {
        ...
        if (this.saveMode)
        {
            ...
            saveName = saveName.ReplaceInsensitive(".png", "").ReplaceInsensitive(".jpg", "")
                .ReplaceInsensitive(".jpeg", "");
    ...
    IEnumerator loadPreview(string filename)
    {
        ...
        bool isImage = lower.EndsWith(".jpg") || lower.EndsWith(".jpeg") || lower.EndsWith(".png") ||
                       lower.EndsWith(".tga") || lower.EndsWith(".bmp") || lower.EndsWith(".tif") ||
                       lower.EndsWith(".tiff");
        ...
        {
            string newFilename = MakeNewThumb(filename, "tempPreview", 512);
            if (newFilename != "null")
            {
        ...
        string[] texts =
        {
            "txt", "rtf", "doc", "xml", "html", "cs", "java", "py", "cpp", "c", "php", "ini", "reg", "odf", "ods",
            "meta"
        };
